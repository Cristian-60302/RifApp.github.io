...
let isAuthenticated = Boolean(localStorage.getItem(ACCESS_KEY_STORAGE));

const $ = (selector) => document.querySelector(selector);
const money = (value) => new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', maximumFractionDigits: 0 }).format(Number(value) || 0);
const amountText = (value) => new Intl.NumberFormat('es-CO', { maximumFractionDigits: 0 }).format(Number(value) || 0);
...
function setAuthenticated(value) { isAuthenticated = value; document.body.classList.toggle('is-logged-in', value); }
async function login(event) {
  event.preventDefault();
  const key = $('#loginPassword').value.trim();
  if (!key) return;
  ...
  try {
    ...
    rememberAccessKey(key); setAuthenticated(true); $('#loginForm').reset(); $('#loginMessage').textContent = ''; await syncStoreFromCloud();
  } catch { $('#loginMessage').textContent = 'Contraseña incorrecta o servicio no disponible.'; }
}
async function syncStoreToCloud(forceCloud = false) {
  ...
  try {
    ...
    if (response.status === 401) { localStorage.removeItem(ACCESS_KEY_STORAGE); setAuthenticated(false); window.alert('La contraseña no es válida.'); }
...
async function syncStoreFromCloud() {
  ...
  try {
    const response = await fetch(API_URL, { headers: { 'X-App-Key': key } });
    if (response.status === 401) { localStorage.removeItem(ACCESS_KEY_STORAGE); setAuthenticated(false); window.alert('La contraseña no es válida.'); return; }
...
function setupMoneyInput(input) { input.type = 'text'; input.inputMode = 'numeric'; input.value = amountText(parseAmount(input.value)); input.addEventListener('input', () => { input.value = amountText(parseAmount(input.value)); }); }
function setupMoneyInputs() { document.querySelectorAll('#ticketPrice, #paidInput, .prize-amount').forEach(setupMoneyInput); }
...
function renderDetail() { const container = $('#ticketDetail'); if (!container) return; if (selectedNumber === null) { container.innerHTML = '<div class="empty-detail"><span class="detail-symbol">⌁</span><h3>Selecciona un número</h3><p>Consulta el estado, asigna un cliente y registra sus abonos.</p></div>'; return; } const raffle = activeRaffle(); const item = ticket(selectedNumber); const balance = Math.max(0, Number(raffle.price) - Number(item.paid || 0)); container.innerHTML = `<div class="detail-heading"><div class="detail-number"><span class="large-chip">${numberText(selectedNumber)}</span><h3>Número ${numberText(selectedNumber)}</h3></div><span class="status-label ${item.status}">${item.status === 'available' ? 'Disponible' : item.status === 'paid' ? 'Pagado' : 'Pendiente'}</span></div><form class="detail-form" id="ticketForm"><label>Cliente<input id="customerInput" required maxlength="60" value="${escapeHtml(item.customer)}" placeholder="Nombre completo"></label><div class="amount-row"><label>Valor boleta<input value="${money(raffle.price)}" disabled></label><label>Abono acumulado<input id="paidInput" type="text" inputmode="numeric" value="${amountText(item.paid || 0)}"></label></div><div class="detail-actions"><button type="submit" class="primary-button">Guardar boleta</button><button type="button" class="secondary-button" id="releaseButton">Liberar</button></div>${item.status !== 'available' ? `<p class="balance-note">Saldo restante: <strong>${money(balance)}</strong></p>` : ''}</form>`; $('#ticketForm').addEventListener('submit', saveTicket); $('#releaseButton').addEventListener('click', releaseTicket); setupMoneyInputs(); }
function saveTicket(event) { event.preventDefault(); const raffle = activeRaffle(); const paid = Math.max(0, Math.min(Number(raffle.price), parseAmount($('#paidInput').value))); raffle.tickets[selectedNumber] = { customer: $('#customerInput').value.trim(), paid, status: paid >= Number(raffle.price) ? 'paid' : 'pending' }; saveStore(); renderDashboard(); }
function releaseTicket() { delete activeRaffle().tickets[selectedNumber]; saveStore(); renderDashboard(); }
function renderRaffleList() { const list = $('#raffleList'); if (!list) return; list.innerHTML = ''; store.raffles.forEach((raffle) => { const item = document.createElement('div'); item.className = `raffle-list-item ${raffle.id === store.activeRaffleId ? 'current' : ''}`; item.innerHTML = `<div><strong></strong><small>${escapeHtml(raffle.reference)} · ${formatDate(raffle.date)}</small></div><div class="raffle-actions"><button class="secondary-button open-raffle" type="button">${raffle.id === store.activeRaffleId ? 'Abierta' : 'Abrir rifa'}</button><button class="delete-raffle" type="button" aria-label="Borrar rifa" title="Borrar rifa">×</button></div>`; item.querySelector('strong').textContent = raffle.name; item.querySelector('.open-raffle').addEventListener('click', () => { store.activeRaffleId = raffle.id; selectedNumber = null; saveStore(); switchView('dashboard'); }); item.querySelector('.delete-raffle').addEventListener('click', () => deleteRaffle(raffle.id)); list.append(item); }); }
function deleteRaffle(id) { const raffle = store.raffles.find((item) => item.id === id); if (!raffle || !window.confirm(`¿Borrar la rifa "${raffle.name}" y toda su información?`)) return; store.raffles = store.raffles.filter((item) => item.id !== id); if (store.activeRaffleId === id) store.activeRaffleId = store.raffles[0]?.id || null; selectedNumber = null; saveStore(true); renderDashboard(); renderHeader(); }
function renderPrizes(prizes = [{ name: 'Primer premio', rule: 'last2', amount: 0 }]) { const list = $('#prizesList'); if (!list) return; list.innerHTML = ''; prizes.forEach((prize, index) => { const row = document.createElement('div'); row.className = 'prize-row'; row.innerHTML = `<span class="prize-index">${index + 1}</span><label>Nombre del premio<input class="prize-name" required value="${escapeHtml(prize.name)}"></label><label>Regla<select class="prize-rule"><option value="last2">Dos últimas cifras</option><option value="first2">Dos primeras cifras</option><option value="middle2">Cifras del medio</option><option value="full">Coincidencia total</option></select></label><label>Valor del premio<input class="prize-amount" type="text" inputmode="numeric" value="${amountText(prize.amount)}"></label><button class="remove-prize" type="button" aria-label="Eliminar premio">×</button>`; row.querySelector('.prize-rule').value = prize.rule; row.querySelector('.remove-prize').addEventListener('click', () => { row.remove(); }); list.append(row); }); setupMoneyInputs(); }
function readPrizes() { return [...document.querySelectorAll('.prize-row')].map((row, index) => ({ id: crypto.randomUUID(), name: row.querySelector('.prize-name').value.trim() || `Premio ${index + 1}`, rule: row.querySelector('.prize-rule').value, amount: parseAmount(row.querySelector('.prize-amount').value) })); }
function saveSetup(event) { event.preventDefault(); const raffle = { id: crypto.randomUUID(), code: `RIFA-${String(store.raffles.length + 1).padStart(3, '0')}`, name: $('#raffleName').value.trim(), date: $('#raffleDate').value, reference: $('#raffleReference').value.trim(), price: parseAmount($('#ticketPrice').value), owner: $('#ownerName').value.trim(), phone: $('#ownerPhone').value.trim(), prizes: readPrizes(), tickets: {}, officialResult: '' }; store.raffles.push(raffle); store.activeRaffleId = raffle.id; selectedNumber = null; saveStore(); $('#setupMessage').textContent = 'Rifa creada correctamente.'; switchView('dashboard'); }
function syncSetupForm() { const raffle = activeRaffle(); $('#raffleForm').reset(); if (!raffle) { renderPrizes(); setupMoneyInputs(); return; } $('#raffleName').value = raffle.name; $('#raffleDate').value = raffle.date; $('#raffleReference').value = raffle.reference; $('#ticketPrice').value = amountText(raffle.price); $('#ownerName').value = raffle.owner; $('#ownerPhone').value = raffle.phone; renderPrizes(raffle.prizes); setupMoneyInputs(); }
function verifyResult() { const result = $('#officialResult').value.replace(/\D/g, ''); const raffle = activeRaffle(); if (result.length !== 4 || !raffle) { setText('#verifiedBadge', 'Ingresa 4 cifras'); return; } raffle.officialResult = result; saveStore(); $('#verifiedBadge').textContent = 'Verificado'; $('#verifiedBadge').classList.add('success'); const winners = $('#winnerResults'); winners.innerHTML = ''; raffle.prizes.forEach((prize, index) => { const winningNumber = prize.rule === 'full' ? result : prize.rule === 'first2' ? result.slice(0, 2) : prize.rule === 'middle2' ? result.slice(1, 3) : result.slice(-2); const match = raffle.tickets[Number(winningNumber)]; const row = document.createElement('div'); row.className = 'winner-result'; row.innerHTML = `<span class="prize-index">${index + 1}</span><div><h4>${escapeHtml(prize.name)}</h4><p>${ruleLabels[prize.rule]} · ${money(prize.amount)}</p></div><div><strong>${winningNumber}</strong><p class="${match ? 'match' : 'no-match'}">${match ? `✓ ${escapeHtml(match.customer)}` : 'Sin boleta registrada'}</p></div>`; winners.append(row); }); renderUnpaidList(); }
function renderUnpaidList() { const list = $('#unpaidList'); const count = $('#unpaidCount'); const raffle = activeRaffle(); if (!list || !count || !raffle) return; const pending = Object.entries(raffle.tickets).filter(([, item]) => item.status !== 'paid'); count.textContent = `${pending.length} ${pending.length === 1 ? 'pendiente' : 'pendientes'}`; list.innerHTML = pending.length ? pending.map(([number, item]) => `<div class="unpaid-row"><span class="large-chip">${numberText(number)}</span><div><strong>${escapeHtml(item.customer)}</strong><small>Saldo pendiente: ${money(Math.max(0, raffle.price - Number(item.paid || 0)))}</small></div></div>`).join('') : '<p class="empty-list">Todas las boletas registradas están pagadas.</p>'; }
function switchView(viewName) { document.querySelectorAll('.view').forEach((view) => view.classList.toggle('active', view.id === `${viewName}View`)); document.querySelectorAll('.nav-item').forEach((item) => item.classList.toggle('active', item.dataset.view === viewName)); if (viewName === 'setup') syncSetupForm(); if (viewName === 'settlement' && activeRaffle()) { $('#officialResult').value = activeRaffle().officialResult; renderUnpaidList(); } renderHeader(); if (viewName === 'dashboard') renderDashboard(); }

document.querySelectorAll('[data-view]').forEach((button) => button.addEventListener('click', () => switchView(button.dataset.view)));
$('#loginForm').addEventListener('submit', login);
$('#logoutButton').addEventListener('click', () => { localStorage.removeItem(ACCESS_KEY_STORAGE); setAuthenticated(false); $('#loginPassword').focus(); });
...
setAuthenticated(isAuthenticated); renderDashboard(); renderHeader(); if (isAuthenticated) syncStoreFromCloud();
