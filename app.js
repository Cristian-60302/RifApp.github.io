...
async function syncStoreToCloud(forceCloud = false) {
  ...
  try {
    ...
    if (response.status === 401) { localStorage.removeItem(ACCESS_KEY_STORAGE); window.alert('La contraseña no es válida.'); }
...
async function syncStoreFromCloud() {
  ...
  try {
    ...
    if (response.status === 401) { localStorage.removeItem(ACCESS_KEY_STORAGE); window.alert('La contraseña no es válida.'); return; }
